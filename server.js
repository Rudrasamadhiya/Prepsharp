...
app.post('/api/login', async (req, res) => {
  ...
  // Rate limiting for failed login attempts (in memory - consider using Redis for production)
  const loginAttempts = {};
  ...
  if (loginAttempts[clientIP] && loginAttempts[clientIP].count >= MAX_ATTEMPTS) {
    const timeLeft = (loginAttempts[clientIP].time + LOCK_TIME) - Date.now();
    if (timeLeft > 0) {
      return res.status(429).json({
        success: false,
        message: `Too many failed attempts. Please try again in ${Math.ceil(timeLeft / 60000)} minutes`
      });
    } else {
  ...
  if (loginMethod === 'otp') {
    ...
    if (!user) {
      ...
      if (!loginAttempts[clientIP]) {
        loginAttempts[clientIP] = { count: 0, time: Date.now() };
      }
      loginAttempts[clientIP].count++;
      loginAttempts[clientIP].time = Date.now();
...
app.listen(PORT, () => console.log(`Server running on port ${PORT}`));) || user.password.startsWith('$2b
...
app.listen(PORT, () => console.log(`Server running on port ${PORT}`));)) {
        ...
        if (passwordValid) {
          try {
            ...
            console.error('Password hash upgrade error:', error);
          }
        }
      }
    }

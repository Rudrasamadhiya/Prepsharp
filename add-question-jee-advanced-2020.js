// JEE Advanced 2020 Question Manager
const jeeAdvanced2020 = {
    // Paper configuration
    paperConfig: {
        paper1: {
            ...
            totalQuestions: 54,
            questionTypes: {
                scq: { count: 6, marks: 3, negative: 1 },
                mcq: { count: 6, marks: 4, negative: 2, partial: true },
                numerical: { count: 6, marks: 4, negative: 0 }
            }
        }
    },
    ...
    // Question counters
    questionCounts: {
        physics: { scq: 0, mcq: 0, numerical: 0 },
        chemistry: { scq: 0, mcq: 0, numerical: 0 },
        mathematics: { scq: 0, mcq: 0, numerical: 0 }
    },
    ...
    initPaper: function(paperKey) {
        ...
        this.currentPaper = {
            id: `jee-adv-2020-${paperKey}-${Date.now()}`,
            type: 'jee-advanced',
            name: config.name,
            year: '2020',
    ...
    savePaper: function() {
        localStorage.setItem('jeeAdvanced2020Paper1', JSON.stringify(this.currentPaper));
    },
    
    // Load paper from localStorage
    loadPaper: function() {
        const savedPaper = localStorage.getItem('jeeAdvanced2020Paper1');
        if (savedPaper) {

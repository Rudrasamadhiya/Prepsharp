// JEE Advanced 2021 Question Manager
const jeeAdvanced2021 = {
    // Paper configuration
    paperConfig: {
        paper1: {
            name: "JEE Advanced 2021: Paper 1",
            subjects: ["physics", "chemistry", "mathematics"],
            questionsPerSubject: 19,
            totalQuestions: 57,
            questionTypes: {
                scq: { count: 4, marks: 3, negative: 1 },
                mcq: { count: 6, marks: 4, negative: 2, partial: true },
                numerical: { count: 3, marks: 4, negative: 0 },
                paragraph: { count: 6, marks: 2, negative: 0 }
            }
        }
    },
    ...
    // Question counters
    questionCounts: {
        physics: { scq: 0, mcq: 0, numerical: 0, paragraph: 0 },
        chemistry: { scq: 0, mcq: 0, numerical: 0, paragraph: 0 },
        mathematics: { scq: 0, mcq: 0, numerical: 0, paragraph: 0 }
    },
    ...
    initPaper: function(paperKey) {
        ...
        this.currentPaper = {
            id: `jee-adv-2021-${paperKey}-${Date.now()}`,
            type: 'jee-advanced',
            name: config.name,
            year: '2021',
    ...
    savePaper: function() {
        localStorage.setItem('jeeAdvanced2021Paper1', JSON.stringify(this.currentPaper));
    },
    
    // Load paper from localStorage
    loadPaper: function() {
        const savedPaper = localStorage.getItem('jeeAdvanced2021Paper1');
        if (savedPaper) {

// JEE Advanced 2022 Question Manager
const jeeAdvanced2022 = {
    // Paper configuration
    paperConfig: {
        paper1: {
            ...
            totalQuestions: 54,
            questionTypes: {
                numerical: { count: 8, marks: 3, negative: 0 },
    ...
    // Question counters
    questionCounts: {
        physics: { numerical: 0, mcq: 0, matchList: 0 },
        chemistry: { numerical: 0, mcq: 0, matchList: 0 },
        mathematics: { numerical: 0, mcq: 0, matchList: 0 }
    },
    ...
    initPaper: function(paperKey) {
        ...
        this.currentPaper = {
            id: `jee-adv-2022-${paperKey}-${Date.now()}`,
            type: 'jee-advanced',
            name: config.name,
            year: '2022',
    ...
    savePaper: function() {
        localStorage.setItem('jeeAdvanced2022Paper', JSON.stringify(this.currentPaper));
    },
    
    // Load paper from localStorage
    loadPaper: function() {
        const savedPaper = localStorage.getItem('jeeAdvanced2022Paper');
        if (savedPaper) {
